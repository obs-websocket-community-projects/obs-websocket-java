...
public class CreateInputRequest extends InputRequest {
    ...
    @Builder
    private CreateInputRequest(String inputName, String inputKind, String sceneName, JsonObject inputSettings, Boolean sceneItemEnabled) {
        super(Type.CreateInput);

        this.requestData = Data.builder().inputName(inputName).inputKind(inputKind).sceneName(sceneName).inputSettings(inputSettings).sceneItemEnabled(sceneItemEnabled).build();
    }
